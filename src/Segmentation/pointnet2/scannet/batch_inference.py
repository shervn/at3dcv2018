...
parser.add_argument('--block_size', type=float, default=1.0, help='the size of the block in x-dir and y-dir by which to process the PC')
parser.add_argument('--stride', type=float, default=1.0, help='the stride block in x-dir and y-dir by which to process the PC')
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
    ...
    current_data, current_label = indoor3d_util.room2blocks_wrapper_normalized(room_path, NUM_POINT, BLOCK_SIZE, STRIDE)
    print('eval one epoch')
    print(np.amin(np.amin(current_data,0),0))
    print(np.amax(np.amax(current_data,0),0))
    ...
    for batch_idx in range(num_batches):
        ...
        for b in range(BATCH_SIZE):
            ...
            for i in range(NUM_POINT):
                ...
                # note: it's the normalized (and then unnormalized?) pts that get written to the pred file
