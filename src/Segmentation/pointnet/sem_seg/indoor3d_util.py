...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    """
    print('start room2blocks')
    print(np.amin(data,0))
    print(np.amax(data,0))

    assert(stride<=block_size)

    limit_min = np.amin(data,0)[0:3]
    ...
    xbeg_list = [] # the final length of these lists is K
    ybeg_list = []
    ...
    block_data_list = np.concatenate(block_data_list, 0)
    block_label_list = np.concatenate(block_label_list, 0)

    print(np.amin(np.amin(block_data_list,0),0))
    print(np.amax(np.amax(block_data_list,0),0))
    print('end room2blocks')
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
    ...
    print(np.amin(data,0))
    print(np.amax(data,0))
    if any(np.amax(data,0)[3:6] > 1): data[:,3:6] /= 255.0
    print(np.amin(data,0))
    print(np.amax(data,0))
    label = data_label[:,-1].astype(np.uint8)
    ...
    print(np.amin(np.amin(new_data_batch,0),0))
    print(np.amax(np.amax(new_data_batch,0),0))
